...
class ImageList(torch.utils.data.Dataset):
    def __init__(self, args, file_name, root_dir, transform=None, type=None):
        ...
        with open(file_name, 'r') as f:
            ...
            for item in lines:
                ...
                self.labels.append(args.source_classes_num if int(line[1].strip()) > args.source_classes_num else int(line[1].strip()))
        self.batch_size = args.batch_size
        self.class_num = args.source_classes_num+1
        self.class_labels = [ [] for i in range(self.class_num)]
        for idx, label in enumerate(self.labels):
            # label = args.source_classes_num if label > args.source_classes_num else label
    ...
    def get_indexes_by_labels(self, labels, per_num):
        ...
        if self.type == 'labeled':
            ...
            for label in labels:
                ...
                indexes += idx
            idx = random.sample(self.pseudo_labels[self.class_num-1], self.batch_size if len(self.pseudo_labels[self.class_num-1]) > self.batch_size else len(self.pseudo_labels[self.class_num-1]))
            indexes += idx
        return indexes
    ...
    def update_pseudo_labels(self, labels):
        ...
        print(len_of_class)
        print('Update pseudo labels. Accuracy is {:.5f}.'.format(100 * correct / total))
